...
contract fBEETSPriceOracle is SpotPriceOracle {
    ...
    constructor(address _fBeetsBar) SpotPriceOracle() {
    ...
    function updateSafePrice() public payable returns (int256) {
       return getSafePriceInternal();
    }
    ...
    function getSafePriceInternal() internal view returns (int256) {
      ...
      uint256 price = bptFtm.mul(bptFBeetsRatio);//.div(1e36);
      return int256(price);
    }
    ...
    function getCurrentPrice() public view override returns (int256) {
        ...
        uint256 vestingTokenLocked = IERC20(vestingToken).balanceOf(fBeetsBar);
        uint256 bptFBeetsRatio = vestingTokenLocked.mul(1e18).div(fBeetsTotalSupply);
        uint256 price = bptFtm.mul(bptFBeetsRatio);
        return int256(price);
    }
