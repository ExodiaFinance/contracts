...
contract OHMCirculatingSupplyContract {
    ...
    function OHMCirculatingSupply() external view returns (uint256) {
        uint256 _totalSupply = IERC20(OHM).totalSupply();

        uint256 _circulatingSupply = _totalSupply.sub(getNonCirculatingOHM());

        return _circulatingSupply;
    }

    function getNonCirculatingOHM() public view returns (uint256) {
        uint256 _nonCirculatingOHM;

        for (uint256 i = 0; i < nonCirculatingOHMAddresses.length; i = i.add(1)) {
            _nonCirculatingOHM = _nonCirculatingOHM.add(
                IERC20(OHM).balanceOf(nonCirculatingOHMAddresses[i])
            );
        }
    ...
    function setNonCirculatingOHMAddresses(address[] calldata _nonCirculatingAddresses)
        external
        returns (bool)
    {
