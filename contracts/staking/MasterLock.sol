...
contract MasterLock is Policy, IOutputReceiver, ERC165, IAddressLock {
    ...
    mapping(uint256 => uint8) lockedBy;
    address[] lockerContracts;
    mapping(uint256 => bool) nftUnlocks;
    ...
    function lock(
        ...
        uint8 lockerId,
        uint8 periodId
    ) external payable contractIdRegistered(lockerId) {
        ...
        uint256 nftId = locker.lock{value: msg.value}(amount, periodId);
        lockedBy[nftId] = lockerId;
    }

    function addToLock(uint256 nftId, uint256 amount)
        external
        onlyNFTOwner(nftId)
        returns (bool)
    {
    ...
    function unlock(uint256 nftId) external onlyNFTOwner(nftId) returns (bool) {
        return true;
    }
    ...
    function unlockNFT(uint256 id) external onlyUnlocker {
        nftUnlocks[id] = true;
    }

    function lockNFT(uint256 id) external onlyUnlocker {
        nftUnlocks[id] = false;
    }
    ...
    modifier onlyNFTOwner(uint256 id) {
        require(
            IFNFTHandler(getRegistry().getRevestFNFT()).getBalance(msg.sender, id) == 1,
            "ML: Caller is not the owner"
        );
        _;
    }

    function getNftLocker(uint256 fnftId) public view returns (address) {
    ...
    function receiveRevestOutput(
        ...
        return
            IOutputReceiver(locker).receiveRevestOutput(fnftId, asset, owner, quantity);
    }
    ...
    function getDisplayValues(uint256 fnftId, uint256 lockId)
        external
        view
        override
        returns (bytes memory)
    ...
    function isUnlockable(uint256 fnftId, uint256 lockId)
        external
        view
        override
        returns (bool)
    {
        address locker = getNftLocker(fnftId);
        return
            isMasterUnlocked(fnftId) || IAddressLock(locker).isUnlockable(fnftId, lockId);
    }

    function isMasterUnlocked(uint256 fnftId) public view returns (bool) {
