...
contract RemoveUniLp is Policy {
    ...
    function withdrawLpFromTreasury(address _lp) private {
        uint256 lpBalance = IERC20(_lp).balanceOf(treasury);
        OlympusTreasury(treasury).manage(_lp, lpBalance);
    }

    function withdrawLpFromPool(address _router, address _lp)
        private
        returns (
            address token0,
            uint256 amount0,
        ...
        )
    {
        ...
        uint256 lpBalance = IUniswapV2Pair(_lp).balanceOf(address(this));
        uint256 lpTotalSupply = IUniswapV2Pair(_lp).totalSupply();
    ...
    function depositInTreasury(address token, uint256 amount) public onlyPolicy {
        IERC20(token).approve(treasury, amount);
        if (canBeDeposited(token)) {
            uint256 value = OlympusTreasury(treasury).valueOf(token, amount);
    ...
    function canBeDeposited(address token) public view returns (bool) {
        return
            OlympusTreasury(treasury).isReserveToken(token) ||
            OlympusTreasury(treasury).isLiquidityToken(token);
    }
}
