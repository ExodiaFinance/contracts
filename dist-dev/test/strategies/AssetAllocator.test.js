...
describe("AssetAllocator", function () {
    ...
    let strategy;
    let mockWinningStrategyFactory;
    let mockLoosingStrategyFactory;
    let mockGreedyStrategyFactory;
    ...
    const setup = deployments.createFixture(async (hh) => {
        ...
        await deployments.fixture([_30_deployAssetAllocator_1.ASSET_ALLOCATOR_DID]);
        ...
        mockStrategyFactory = await smock_1.smock.mock("MockStrategy");
        strategy = await mockStrategyFactory.deploy();
        mockTokenFactory = await smock_1.smock.mock("MockToken");
        mockWinningStrategyFactory = await smock_1.smock.mock("MockWinningStrategy");
        mockLoosingStrategyFactory = await smock_1.smock.mock("MockLoosingStrategy");
        mockGreedyStrategyFactory = await smock_1.smock.mock("MockGreedyStrategy");
    });
    ...
    */
    describe("Rebalancing", function () {
        const mintAmount = (0, utils_1.parseUnits)("100000", "ether");
        let daiTreasuryBalance0;
        let totalReserves0;
        ...
        describe("To a single strategy", function () {
            ...
            it("Should return the balance deployed", async function () {
                ...
                (0, chai_1.expect)(balance).to.eq(deployedAmount);
            });
            it("Should send the fund in the strategy", async function () {
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(deployedAmount);
                (0, chai_1.expect)(await dai.balanceOf(strategy.address)).to.eq(
                    deployedAmount
                );
            });
            ...
            it("Should return all funds", async function () {
                ...
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(mintAmount);
                const stratBalance = await dai.balanceOf(strategy.address);
                (0, chai_1.expect)(stratBalance).to.eq(0);
                (0, chai_1.expect)(strategy.deploy).to.have.been.calledWith(dai.address);
            });
            it("Should return all funds", async function () {
            ...
            it("Should return funds if allocation reduced", async function () {
                ...
                (0, chai_1.expect)(strategy.withdrawTo).to.have.been.calledWith(
                    dai.address,
                    deployedAmount.sub(deployedAmount1),
                    deployer
                );
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(
                    mintAmount.sub(deployedAmount1)
                );
                const stratBalance = await dai.balanceOf(strategy.address);
                (0, chai_1.expect)(stratBalance).to.eq(deployedAmount1);
            });
            it("Should add funds", async function () {
                ...
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(
                    mintAmount.sub(targetAmount)
                );
                const stratBalance = await dai.balanceOf(strategy.address);
                (0, chai_1.expect)(stratBalance).to.eq(targetAmount);
            });
        ...
        describe("To a single profitable strategy", function () {
            const profits = (0, utils_1.parseUnits)("100", "ether");
            ...
            it("Should return deposited and profits", async function () {
                const balance = await assetAllocator.allocatedBalance(dai.address);
                (0, chai_1.expect)(balance).to.eq(deployedAmount0.add(profits));
            });
            ...
            it("Should withdraw profits", async function () {
                ...
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(profits);
            });
            it("Should add on top of profits", async function () {
                const newDeposit = (0, utils_1.parseUnits)("1000", "ether");
        ...
        describe("To an unprofitable strategy", function () {
            ...
            it("Should not fail allocating more if accounting for loss", async function () {
                ...
                (0, chai_1.expect)(await loosingStrat.balance(dai.address)).to.eq(
                    deployedAmount
                        .mul(returnRate)
                        .div(100)
                        .add(newDeposit)
                ...
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(0);
            });
            it("Should withdraw everything", async function () {
                ...
                await assetAllocator.rebalance(dai.address, deployedAmount);
                (0, chai_1.expect)(await loosingStrat.balance(dai.address)).to.eq(0);
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(
                    deployedAmount.mul(returnRate).div(100)
                );
            });
        ...
        describe("To multiple strategies", function () {
            ...
            const amountAllocReturned = (returnRate) => (allocTo, balance) =>
                balance.mul(returnRate).div(100).mul(allocTo).div(maxAlloc);
            const amountAlloc = amountAllocReturned(100);
            const loss = (lossPercent) => (amount) => amount.mul(lossPercent).div(100);
            const expectProperAllocation = async (
                amountStrat0,
                amountStrat1,
                balance = 0
            ) => {
                (0, chai_1.expect)(await dai.balanceOf(deployer)).to.eq(balance);
                const strat0Balance = await strat0.balance(dai.address);
                (0, chai_1.expect)(strat0Balance).to.eq(amountStrat0);
                const strat1Balance = await strat1.balance(dai.address);
                (0, chai_1.expect)(strat1Balance).to.eq(amountStrat1);
            };
            const testAllocations = (balance0, balance1, loss0, loss1) => {
                it("Should return total balance", async function () {
                    (0, chai_1.expect)(
                        await assetAllocator.allocatedBalance(dai.address)
                    ).eq(balance0(alloc0, mintAmount).add(balance1(alloc1, mintAmount)));
                });
                ...
                it("Should reallocate higher-lower", async function () {
                    const newAlloc0 = 40000;
                    const newAlloc1 = 60000;
                    await allocationCalculator.setAllocation(
                        dai.address,
                ...
                it("Should reallocate lower-higher", async function () {
                    const newAlloc0 = 10000;
                    const newAlloc1 = 90000;
                    await allocationCalculator.setAllocation(
                        dai.address,
                        [strat0.address, strat1.address],
                        [newAlloc0, newAlloc1]
    ...
    describe("CollectRewards", function () {
        const amount0 = (0, utils_1.parseUnits)("1", "ether");
        const amount1 = (0, utils_1.parseUnits)("3", "ether");
        const amount2 = (0, utils_1.parseUnits)("7", "ether");
        const amount3 = (0, utils_1.parseUnits)("12", "ether");
        let tok0;
        let tok1;
        let tok2;
        let strat0;
        ...
        const setupRewards = deployments.createFixture(async (hh) => {
            ...
            const rewardStratFactory = await smock_1.smock.mock("MockRewardingStrategy");
        ...
        it("Should collect rewards and deposit in treasury", async function () {
            ...
            (0, chai_1.expect)(await tok0.balanceOf(treasury.address)).to.eq(amount0);
            (0, chai_1.expect)(await tok1.balanceOf(treasury.address)).to.eq(
                amount1.add(amount2)
            );
            (0, chai_1.expect)(await tok2.balanceOf(treasury.address)).to.eq(amount3);
        });
    });
    describe("CollectProfits", function () {
        ...
        const setupRewards = deployments.createFixture(async (hh) => {
            tok0 = await mockTokenFactory.deploy(18);
            const profitsStratFactory = await smock_1.smock.mock(
        ...
        it("Should collect rewards and deposit in treasury", async function () {
            ...
            (0, chai_1.expect)(await tok0.balanceOf(treasury.address)).to.eq(
                amount0.add(amount1)
            );
        });
    });
    describe.only("Allocate", function () {
        ...
        const amount0 = (0, utils_1.parseEther)("1000");
        const amount1 = (0, utils_1.parseEther)("200");
        const setupStrats = deployments.createFixture(async (hh) => {
            tok0 = await mockTokenFactory.deploy(18);
            const loosingStratFactory = await smock_1.smock.mock("MockStrategy");
            strat0 = await loosingStratFactory.deploy();
            strat0.balance.returns((0, utils_1.parseEther)("1")); // sets non-sense value for balance, cause it's not supposed ot be used
            strat1 = await loosingStratFactory.deploy();
            strat0.balance.returns((0, utils_1.parseEther)("10000"));
            await allocationCalculator.setAllocation(
                tok0.address,
                [strat0.address, strat1.address],
                [alloc0, alloc1]
        ...
        it("Should allocate", async function () {
            (0, chai_1.expect)(await tok0.balanceOf(strat0.address)).to.eq(
                amount0.mul(alloc0).div(totalAlloc)
            );
            (0, chai_1.expect)(await tok0.balanceOf(strat1.address)).to.eq(
        ...
        it("Should not withdraw", async function () {
            ...
            (0, chai_1.expect)(await tok0.balanceOf(strat0.address)).to.eq(bal0);
            (0, chai_1.expect)(await tok0.balanceOf(strat1.address)).to.eq(bal1);
        });
    });
});
